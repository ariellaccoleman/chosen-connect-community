...
const ChatPage = () => {
  ...
  const [activeChannelId, setActiveChannelId] = useState<string | null>(isValidChannelId(channelId) ? channelId : null);
  ...
  return <div className="h-[calc(100vh-64px)] flex flex-col sm:flex-row bg-gray-50 dark:bg-gray-900">
      ...
      <div className={`flex-1 flex flex-col overflow-hidden ${isThreadOpen ? 'hidden md:flex' : ''}`}>
        {isValidChannelId(activeChannelId) ? <MessageFeed channelId={activeChannelId} onMessageSelect={handleMessageSelect} selectedMessageId={selectedMessage?.id} /> : <div className="flex-1 flex items-center justify-center p-6 text-center bg-white dark:bg-gray-800">
            <div>
      ...
      {isThreadOpen && selectedMessage && <div className="w-full md:w-80 lg:w-96 border-l border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 flex flex-col overflow-hidden">
          <ThreadPanel parentMessage={selectedMessage} channelId={activeChannelId || ''} onClose={handleCloseThread} />
        </div>}
    </div>;
};
const Chat = () => {
  return <ErrorBoundary name="ChatPage">
      <ChatPage />
    </ErrorBoundary>;
};
export default Chat;
